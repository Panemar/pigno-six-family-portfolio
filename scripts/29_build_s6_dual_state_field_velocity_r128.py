#!/usr/bin/env python3
"""Final rank-level audit: keep displacement R64 and raise only velocity to R128."""
...
PREVIOUS = ROOT / "s6_micropanel_common" / "S6_DUAL_STATE_FIELD_REPRESENTATION.h5"
OUTPUT = ROOT / "s6_micropanel_common" / "S6_DUAL_STATE_FIELD_REPRESENTATION_VELOCITY_R128.h5"
REPORT = ROOT / "s6_micropanel_common" / "S6_DUAL_STATE_FIELD_REPRESENTATION_VELOCITY_R128_REPORT.json"
...
def project(fields: np.ndarray, bases: np.ndarray) -> tuple[np.ndarray, list[list[float]]]:
    ...
    for case in range(fields.shape[0]):
        ...
        for axis in range(3):
            ...
            case_errors.append(rel(coefficients[case, :, axis] @ bases[axis].T, fields[case, :, :, axis]))
        errors.append(case_errors)
    return coefficients, errors


def main() -> None:
    for path in (DATASET, CAPACITY, PREVIOUS):
        if not path.is_file():
    ...
    with h5py.File(PREVIOUS, "r") as handle:
        displacement_basis = np.asarray(handle["displacement/basis_by_axis"][:], dtype=np.float64)
        displacement_coeff = np.asarray(handle["displacement/coefficients"][:], dtype=np.float64)
    velocity_basis = basis(velocity, VEL_RANK)
    velocity_coeff, velocity_errors = project(velocity, velocity_basis)
    _, displacement_errors = project(displacement, displacement_basis)
    ...
    ) else "FAIL_S6_DUAL_STATE_FIELD_REPRESENTATION_AFTER_VELOCITY_R128"
    ...
    with h5py.File(temporary, "w") as handle:
        handle.attrs.update(
            status=status,
            schema="S6_DUAL_STATE_FIELD_REPRESENTATION_VELOCITY_R128_V1",
    ...
    report = {
        "schema": "S6_DUAL_STATE_FIELD_REPRESENTATION_VELOCITY_R128_REPORT_V1",
        "status": status,
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "change_from_previous": "displacement and Physical32 frozen; velocity field rank per axis increased from 64 to 128",
        ...
        "scientific_boundary": {
            ...
            "fold_local_rebuild_required_later": True,
        },
        "source_hashes": {str(path): sha256(path) for path in (DATASET, CAPACITY, PREVIOUS)},
